...
begin
  Assign(InputFile, 'input.txt'); // открываем файл для чтения
  Reset(InputFile);

  Assign(OutputFile, 'output.txt'); // открываем файл для записи
  Rewrite(OutputFile);

  InitStack(Stack); // инициализируем стек
  
  
  writeln('Содержимое файла input.txt: ');
  // читаем содержисое файла input.txt
  while not Eof(InputFile) do // пока не достигнут конец файла
  begin
    Readln(InputFile, Line); // читаем строку из файла
    writeln(Line); // выводис строку
  end;
  ...
  writeln();
  writeln('Содержимое файла output.txt: ');
  // читаем содержисое файла output.txt
  Assign(OutputFile, 'output.txt'); // открываем файл для чтения
  Reset(OutputFile);
  while not Eof(OutputFile) do // пока не достигнут конец файла
  begin
    Readln(OutputFile, Line); // читаем строку из файла
    writeln(Line); // выводис строку
  end;
  Close(OutputFile);// закрываем входной файл
