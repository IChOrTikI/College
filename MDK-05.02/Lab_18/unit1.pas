...
function MyFunc(i: string):real;   //Объявлвем функцию MyFunc кторая принимает перменную i типа строка
...
begin
  ...
  //Можно весто result писать имя функции (Result:=i * 2;) или (MyFunc:=i * 2;)
end;

procedure UdvoeniePoSsilke(var r: real);   // Создаем процедуру  кторая принимает переменную с типом real
...
procedure TfMain.MyPrivate;   //Данэная процедура кторая обявлена  предворительно
