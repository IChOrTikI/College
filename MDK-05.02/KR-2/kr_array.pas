...
procedure InsertBeforeElement(value, targetValue: Integer); // Процедура для вставки элемента перед указанным элементом

implementation

procedure InsertBeforeElement(value, targetValue: Integer);
var
  current, newIndex: Integer;
begin
  current := head; // Устанавливаем текущий индекс на начало списка

  while current <> 0 do // Пока не конец списка
  begin
    if list[current].data = targetValue then // Находим целевой элемент
    begin
      if size >= MAX_SIZE then // Проверяем, не переполнен ли список
      begin
        WriteLn('Список полон'); // Выводим сообщение об ошибке
        Exit; // Выходим из процедуры
      end;

      newIndex := size + 1; // Увеличиваем индекс для нового узла
      list[newIndex].data := value; // Присваиваем значение новому узлу

      if current = head then // Если целевой элемент - начало списка
      begin
        list[newIndex].prev := 0; // Предыдущий узел нового элемента равен 0
        list[newIndex].next := current; // Следующий узел нового элемента - текущий
        list[current].prev := newIndex; // Предыдущий узел текущего элемента - новый
        head := newIndex; // Устанавливаем новый элемент как начало списка
      end
      else // Если целевой элемент не начало списка
      begin
        list[newIndex].prev := list[current].prev; // Предыдущий нового элемента - предыдущий текущего
        list[newIndex].next := current; // Следующий нового элемента - текущий
        list[list[current].prev].next := newIndex; // Следующий предыдущего узла - новый элемент
        list[current].prev := newIndex; // Предыдущий текущего элемента - новый элемент
      end;

      Inc(size); // Увеличиваем размер списка
      WriteLn('Элемент ', value, ' добавлен перед элементом ', targetValue); // Выводим сообщение об успешном добавлении
      Exit; // Выходим из процедуры
    end;
  ...
  WriteLn('Элемент с значением ', targetValue, ' не найден.'); // Если элемент не найден
end;
...
var
  choice, value, targetValue: Integer; // Переменные для выбора, значения и целевого значения
begin
  head := 0; // Обнуляем индекс начала списка
  tail := 0; // Обнуляем индекс конца списка
  size := 0; // Обнуляем размер списка
  ...
  repeat
    ...
    WriteLn('3. Удалить элемент по значению');
    WriteLn('4. Очистить список');
    WriteLn('5. Добавить элемент перед указанным элементом');
    WriteLn('6. Выход');
    Write('Введите ваш выбор: ');
    ...
    case choice of
      ...
      begin
        Write('Введите значение, которое хотите добавить: ');
        ReadLn(value); // Считываем значение для добавления
        Write('Введите элемент, перед которым хотите добавить значение: ');
        ReadLn(targetValue); // Считываем значение целевого элемента
        InsertBeforeElement(value, targetValue); // Вызываем процедуру добавления элемента перед указанным элементом
      end;
      else
        WriteLn('Ошибка выбора. Попробуйте снова'); // Выводим сообщение об ошибке выбора
    end;

  until choice = 6; // Повторяем цикл до выбора выхода
end;
end.
